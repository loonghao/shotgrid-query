...
def _validate_filter_structure(filter_item: Any, index: int) -> list[str]:
    ...
    if not isinstance(filter_item, (list, tuple)) or len(filter_item) != 3:
        errors.append(f"Filter {index + 1} must be a list/tuple with exactly 3 elements [field, operator, value]")
        ...
        if " " in value:
            try:
                ...
                if unit.lower() not in unit_map:
                    errors.append(
                        f"Filter {index + 1}: Invalid time unit '{unit}'. "
                        f"Valid units are: day(s), week(s), month(s), year(s)"
                    )
